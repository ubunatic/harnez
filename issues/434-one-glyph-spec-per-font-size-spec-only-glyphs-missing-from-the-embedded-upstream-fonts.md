...
- **M1**: Embed the BDFs and add a BDF reader in `internal/readcard` (reuse the baseline and bounding-box logic from `scripts/import-bdf-font.go`), loaded lazily per size. Verify: unit tests on a fixture BDF; `harnez --version` timing unchanged.
