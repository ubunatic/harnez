...
- **Binary / PATH Shim vs. Shell Alias**:
  ...
  - **Binary Dispatch**: The `harnez` binary detects if `os.Args[0]` (or `filepath.Base(os.Args[0])`) is `⚙`, automatically behaving as `harnez exec "$@"`.
...
- **Lifecycle & Distribution**:
  - Installed or updated during `harnez apply` and `make install` across managed agent directories (`~/.claude/bin/⚙`, `~/go/bin/⚙`, `~/.local/bin/⚙`, `~/.prime/agent/bin/⚙`).
...
- Document usage of `⚙` in agent docs / command references (e.g. `docs/HookRewritePattern.md`).
...
- [x] Invoking `⚙ <cmd>` or `⚙ [flags] -- <cmd>` behaves identically to `harnez exec [flags] -- <cmd>`.
- [x] `harnez apply` provisions the `⚙` shim/symlink in managed agent environments / PATH so it is immediately usable by agents.
- [x] Multicall/symlink dispatch works when `harnez` binary is symlinked to `⚙`.
- [x] Flags such as `--tool`, `--distill`, and `--expect-failure` are correctly forwarded through `⚙`.
- [x] Unit tests cover `⚙` symlink/dispatch invocation and argument handling.
- [x] Documentation reflects `⚙` as a supported compact alias for `harnez exec` in agent workflows.

## 5. Verification Guidance

- **Unit test**: Test `os.Args[0]` multicall dispatch and symlink execution in Go tests (`TestIsGearInvocation`, `TestExecCmd_GearAliasAndFlags`, `TestGearMulticallExecution`, `TestGearSymlinkProvisioning`).
- **Live verification**:
  1. Ran `make install` and `harnez apply`.
  2. Ran `⚙ echo "hello from gear"` in bash and verified exit code 0 and stdout `hello from gear`.
  3. Ran `⚙ --tool test -- true` and verified telemetry row recording in `harnez stats`.
  4. Ran `scripts/smoke-test.sh` — verified drift simulation, repair, idempotency, and clean exits.
