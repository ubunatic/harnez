# 351 — Add AGENTS.md pointer: check config.yaml/Spec.md before hardcoding named business-value lists in Go
...
Add a short line to this project's `AGENTS.md`:
...
`AGENTS.md` is — an agent has to already know to consult it. A one-line pointer in
`AGENTS.md` would surface the check *before* the violation happens, not just document
it afterward for the next reader who happens to open `Spec.md`.

## Task

Add the note above to this project's `AGENTS.md` (harnez-managed conventions block, or
wherever similar cross-references to `docs/Spec.md` already live — check how other
`@docs/` references are wired into the managed `AGENTS.md` sections in `config.yaml`'s
`agents_md` config, since this project generates its own `AGENTS.md` from that config
rather than hand-editing it directly).

## Definition of done

- The pointer line is live in the actual installed `AGENTS.md` targets (via `harnez
  apply`), not just in this repo's source template — confirm with `harnez status` or a
  direct read after applying.
- Doesn't duplicate `docs/Spec.md`'s full content — just points to it, matching how
  other `@docs/` references in `AGENTS.md` behave.
