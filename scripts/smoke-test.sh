...
then
    echo "  SKIP: harnez not found on PATH — run 'make install' first"
else
    ...
    then
        fail "harnez usage --summary --compact produced no output"
    fi
    ...
    do
        ...
        then
            ...
            if test -n "$row"
            then
                all_usage_rows="${all_usage_rows}${row}
"
            fi
        fi
    done <<EOF
...
EOF
    ...
    do
        ...
        if test "$col" -lt 0
        then
            echo "  WARN: row $row_num has no second bar, skipping: $row"
            continue
        fi
        ...
        then
            echo "  FAIL: bar alignment broken on row $row_num" >&2
...
EOF
    ...
    then
        pass "all [a] All Usage bars align at column $expected_col"
    else
        fail "bar column alignment is broken (see above)"
    fi
fi
