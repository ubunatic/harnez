...
func TestReviewSkillInstallToEveryConfiguredTarget(t *testing.T) {
	...
	for _, skillsRoot := range skillTargets(cfg) {
		path := filepath.Join(skillsRoot, "review", "SKILL.md")
		content := readReviewInstall(t, path)
		if !strings.HasPrefix(content, "---\nname: \"review\"\ndescription:") {
			t.Errorf("expected skill frontmatter in %s, got:\n%s", path, content)
		}
		assertReviewRequirements(t, path, content)
	}
...
func readReviewInstall(t *testing.T, path string) string {
	t.Helper()
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("expected review workflow at %s: %v", path, err)
	}
	return string(data)
}

func assertReviewRequirements(t *testing.T, path, content string) {
	t.Helper()
	for _, want := range []string{
		"Review Current Changes",
		"Verify Issue Completion",
		"Check Code Quality",
		"Do Small Fixes Directly",
		"File Bigger Issues as New Issues",
	} {
