...
| [HookRewritePattern.md](HookRewritePattern.md) | Per-agent routing into `harnez exec` (agy: quiet shim via `harnez-agy`, hook fallback, coverage view; command output size and agy truncation) and the two-stage PreToolUse hook pattern (rewrite now, capture later): `<feature> hook` vs. `<feature>` wrapper, `harnez distill hook` as reference implementation, plus two hard-won constraints — hooks on the same matcher don't compose (last-to-finish wins) and a rewritten command must stay one shell token (consult before adding any new agent-hook-driven feature) |
...
| [ModelResearch.md](ModelResearch.md) | Repeatable web-research plan for model guidance: triggers, id check, one researcher per model family plus one price/efficiency researcher, claim ledger, reconcile, spec/snapshot update, cleanup (consult when a new model arrives or prices change) |
| [ModelTrials.md](ModelTrials.md) | Cheap repo-local model trials (Go/TUI/SQLite canaries, EFF from the same runs, seeded-bug reviewer test) for cells web research leaves open; budget rules and what is not worth running |
