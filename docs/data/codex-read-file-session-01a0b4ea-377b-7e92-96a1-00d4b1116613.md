...
I’ll inspect the repository’s local overlay and Makefile, keeping the read focused.

## Activity

    $ /usr/bin/zsh -lc "⚙ bash -c 'sed -n '\\''1,240p'\\'' AGENTS.local.md && printf '\\''\\n--- Makefile ---\\n'\\'' && sed -n '\\''1,260p'\\'' Makefile'"
...
Read [Makefile](/home/uwe/projects/harnez/Makefile). It defines build, install, apply/init, checks, smoke tests, release, sync, and Quota-1 test targets.
...
I’ll enumerate `docs/lang`, then read each document and condense it to one sentence.

## Activity

    $ /usr/bin/zsh -lc "⚙ bash -c 'rg --files docs/lang | sort | while read -r f; do wc -l \""'$f"; done'"'"
    status: Completed · exit 0
...
## Activity
    ...
    $ /usr/bin/zsh -lc "⚙ bash -c 'for f in docs/lang/*.md; do printf '\\''\\n===== %s =====\\n'\\'' \""'$f"; sed -n '"'\\''1,220p'\\'' \""'$f"; done'"'"
    status: Completed · exit 0
...
## Activity
    ...
    $ /usr/bin/zsh -lc "⚙ bash -c 'for f in docs/lang/Go.md docs/lang/Make.lite.md docs/lang/Make.md docs/lang/Git.md; do printf '\\''\\n===== %s =====\\n'\\'' \""'$f"; sed -n '"'\\''1,280p'\\'' \""'$f"; done'"'"
    status: Completed · exit 0
...
## Activity
    ...
    $ /usr/bin/zsh -lc "⚙ bash -c 'for f in docs/lang/Bash.md docs/lang/Bash.lite.md docs/lang/Git.md docs/lang/Zig.md docs/lang/ManPages.md; do printf '\\''\\n===== %s (tail) =====\\n'\\'' \""'$f"; tail -n 70 "$f"; done'"'"
    status: Completed · exit 0
      
      ===== docs/lang/Bash.md (tail) =====
...
- `Bash.md` — Defines strict Bash style: `set -euo pipefail`, explicit `source`, `if test`, quoted expansions, safe traps, timeouts, and directory scoping.
- `Bash.lite.md` — Condensed Bash rules covering conditionals, quoting, output, traps, alignment, `-C` usage, and mawk portability.
- `Cpp.md` — Recommends ccache, serialized CGO builds, and constrained Go memory settings for heavy C/C++ compilation.
- `GTK4.md` — Documents PyGObject/GTK4 patterns for application uniqueness, gesture handling, drawing overlays, and integration smoke tests.
- `Git.md` — Establishes default-branch workflows, proactive conventional commits, review requirements, hygiene rules, and push restrictions.
- `Go.md` — Sets Go standards for minimal dependencies, project layout, workspaces, specs, Cobra CLIs, error handling, Unicode-aware output, and table-driven tests.
- `Make.md` — Defines self-documenting Makefiles using `help`, sentinel phony targets, dependency-driven actions, local binaries, checks, and deployment parity.
- `Make.lite.md` — Condensed Make conventions covering target structure, sentinel phony markers, build/install/check patterns, and remote deployment targets.
- `ManPages.md` — Describes generating and installing Cobra-derived man pages from the CLI, including paths, packaging, and flag-collision pitfalls.
- `Markdown.md` — Specifies naming conventions, concise document structure, hyperlink usage, and appropriate diagram formats for chat versus documentation.
- `Rust.md` — Recommends stable Rust, standard tooling, minimal dependencies, `Result`-based errors, conventional project layout, and documented minimal unsafe code.
- `Zig.md` — Records Zig 0.16 allocation rules, optimization goals, namespace practices, and version-sensitive APIs and interoperability pitfalls.
...
## Activity
    ...
    status: Completed · exit 0
      TOOL  CALLS  AVG SCORE  AVG TOKENS  SAVINGS TOKENS  SAVINGS BYTES  FAILURE RATE
      Bash  8      2.33       16489.6     -               -              25.0%
      rg    1      5.00       16072.0     -               -              0.0%
      sed   1      5.00       n/a         -               -              0.0%
      
      AGENT  CALLS  AVG SCORE  AVG TOKENS  SAVINGS TOKENS  SAVINGS BYTES  FAILURE RATE
      codex  10     3.40       16420.0     -               -              20.0%
      
      PROJECT  CALLS  AVG SCORE  AVG TOKENS  SAVINGS TOKENS  SAVINGS BYTES  FAILURE RATE
      harnez   10     3.40       16420.0     -               -              20.0%
