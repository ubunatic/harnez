...
6. **Context Discipline & Range-Bounded Ingestion**:
   ...
   - **File Read Tool Discipline**: Do NOT use repetitive native IDE file-read tools (`view_file`, `View`, `ReadMultipleFiles`) for whole-file reads or multi-slice explorations on medium/large files (>100 lines). Always execute `harnez read -I <file>` (dense visual PNG context card with retro pixel font) or `harnez read -L <range>` / `harnez read -n` via command execution to prevent token bloat and rate-limit exhaustion.
